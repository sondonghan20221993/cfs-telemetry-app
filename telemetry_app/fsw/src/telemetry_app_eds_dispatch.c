...
#include "telemetry_app_eds_dispatcher.h"
#include "telemetry_app_eds_dictionary.h"

/*
 * Define a lookup table for Telemetry App command codes
 */
/* clang-format off */
static const EdsDispatchTable_EdsComponent_TELEMETRY_APP_Application_CFE_SB_Telecommand_t TELEMETRY_APP_TC_DISPATCH_TABLE =
{
    .CMD =
    {
        .NoopCmd_indication          = TELEMETRY_APP_NoopCmd,
        .ResetCountersCmd_indication = TELEMETRY_APP_ResetCountersCmd
    },
    .SEND_HK =
    {
        .indication = TELEMETRY_APP_SendHkCmd
    }
};
/* clang-format on */
...
void TELEMETRY_APP_TaskPipe(const CFE_SB_Buffer_t *SBBufPtr)
{
    ...
    Status = EdsDispatch_EdsComponent_TELEMETRY_APP_Application_Telecommand(SBBufPtr, &TELEMETRY_APP_TC_DISPATCH_TABLE);
    ...
    if (Status != CFE_SUCCESS)
    {
        ...
        {
            CFE_EVS_SendEvent(TELEMETRY_APP_CC_ERR_EID, CFE_EVS_EventType_ERROR,
                              "Telemetry App: Invalid ground command code: CC = %d", (int)MsgFc);
        }
    }
}
